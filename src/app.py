...
st.markdown("<h6 style='text-align: left; color: black;'>Кому нужно A/B-тестирование</h6>", unsafe_allow_html=True)
st.write(
	"""

	1. _Бизнес аналитики_ для поиска проблемы в бизнесе, оптимизации бизнес-процессов компаний и помощи в запуске новых продуктов компании.
    
	2. _Аналитики данных_ могут анализировать данные различной природы и проверять свои гипотезы при помощи A/B-тестирования.
    
    3. _Продакт-менеджеры_ могут тестировать изменения ценовых моделей, направленные на повышение доходов, или оптимизацию части воронки продаж для увеличения конверсии.
	
	4. _Маркетологи_ могут тестировать изображения, призывы к действию (call-to-action) или практически любые другие элементы маркетинговой кампании или рекламы с точки зрения улучшения метрик.
	
	5. _Продуктовые дизайнеры_ могут тестировать дизайнерские решения (например, цвет кнопки оформления заказа) или использовать результаты тестирования для того, чтобы перед внедрением определить, будет ли удобно пользоваться новой функцией.
    
    6. _Для студентов других специальностей_. Для общего понимания современных технологий в сфере анализа данных.
	"""
)

st.markdown("<h6 style='text-align: left; color: black;'>Почему A/B-тестирование важно</h6>", unsafe_allow_html=True)
st.write(
	"""
    1. _Позволяет_ проверить любую гипотезу на реальных пользователях без существенной потери прибыли.
    
    2. _Позволяет_ повысить доходы за счет введения нового инструмента.
    
    3. _Позволяет_ находить скрытые взаимосвязи между тестируемой гипотезой и другими.
    
    4. _Порой_ даже незначительные изменения могут повлечь за собой большие изменения прибыли.
	"""
)

st.markdown("<h6 style='text-align: center; color: black;'>Выберите вариант рекламного баннера, на который, на ваш счет, активнее кликают</h6>", unsafe_allow_html=True)

st.image('images/ab-example.png', width=700)

click = st.selectbox("Активнее кликают", ["", "Первый вариант", "Второй вариант"])

if click == "Первый вариант":
    st.write("""Поздравляем! Вы верно выбрали рекламный баннер. Согласно исследованиям, его CTR составляет 5,2% (по сравнением с CTR второго варианта баннера - 4,12%). Отличие в 1.08% является очень существенным. Так как, в реальной жизни, повышение CTR даже на 0,5% является отличным результатом.""")
    st.write("""CTR - метрика в интернет-маркетинге. Определяется как отношение числа кликов на баннер или 
рекламное объявление к числу показов, измеряется в процентах.""")
elif click == "Второй вариант":
    st.write("""К сожалению, Вы ошиблись! Первый баннер пользуется большей популярностью у большинства пользователей. Согласно исследованиям, его CTR составляет 5,2% (по сравнением с CTR второго варианта баннера - 4,12%). Отличие в 1.08% является очень существенным. Так как, в реальной жизни, повышение CTR даже на 0,5% является отличным результатом.""")
    st.write("""CTR - метрика в интернет-маркетинге. Определяется как отношение числа кликов на баннер или 
рекламное объявление к числу показов, измеряется в процентах.""")
    
st.markdown("<h2 style='text-align: left; color: black;'>Этапы разработки кейса</h2>", unsafe_allow_html=True)
st.image('images/ab-stages.png', width=700)

st.markdown("<h6 style='text-align: center; color: black;'>Этапы проведения A/B-тестирования</h6>", unsafe_allow_html=True)
st.markdown(
	"""
...
with st.expander('Шаг 1. Определите цели', expanded=True):
	...
	if purpose == 'Занять делом скучающих сотрудников':
		st.error(
			"""
			Этой цели мы безусловно добьемся, но бизнесу от этого легче не станет.
			"""
		)
	elif purpose == 'Решить проблему пользователей':
		st.info(
			"""
	...
	elif purpose == 'Снизить риски при значительных изменениях':
		st.info(
			"""
	...
	elif purpose == 'Обеспечить статистически значимые улучшения':
		st.info(
			"""
...
if STEP_2:
	with st.expander('Шаг 2. Определите метрику', expanded=True):
		st.write(
			...
			Всегда важно понимать подходит ли выбранная Вами метрика для данного эксперимента. Для этого проводят
			A/A–тестирование. Выборка все также делится на две части, но в этот раз обеим выборкам показывается 
			первоначальный сервис, без изменений. Если выбранная Вами метрика не показала различий между выборками, значит она
			Вам подходит.
			""")
		...
		if metrick == 'Обеспечить лучшую окупаемость инвестиций (ROI)':
			st.info(
				"""
		...
		elif metrick == 'Уменьшить показатель отказов':
			st.info(
				"""
		...
		elif metrick == 'Повысить конверсию':
			st.info(
				"""
...
if STEP_4:
	with st.expander('Шаг 4. Подготовьте эксперимент', expanded=True):
		st.write(
			...
			4. _Выбрать статистический тест._
			
				Данный тест позволит нам определить уровень статистической значимости в нашем эксперименте. В рамках данной 
				лабораторной работы мы будем рассматривать Z-тест.
				"""
		)

		st.image('images/z-test.png', width=670)

		st.write(
			"""
			С различными вариантами статистических тестов можно ознакомиться по ссылке [здесь](https://habr.com/ru/post/556852/?ysclid=l6tmtxyav0644934525).
			"""
		)

		st.write(
			"""
			5. _Определите уровень статистической значимости (α)_.
			...
			6. _Определите минимальный размер выборки_. Калькулятор есть [здесь](https://www.evanmiller.org/ab-testing/sample-size.html).
			...
			7. _Определите временные рамки_. Калькулятор есть [здесь](https://vwo.com/tools/ab-test-duration-calculator/).
...
click0 = st.selectbox("Что такое CTR?", ["", "Процент пользователей, которые совершили целевое действие по отношению к общему числу посетителей", "Метрика, показывающая как часто покупают определенный товар", "Метрика в интернет-маркетинге. Определяется как отношение числа кликов на баннер или рекламное объявление к числу показов"])

click1 = st.selectbox("Что такое конверсия?", ["", "Метрика, которая определяется как отношение числа кликов на баннер или рекламное объявление к числу показов, измеряется в процентах", "Процент пользователей, которые совершили целевое действие по отношению к общему числу посетителей", "Процент пользователей купивших наш товар"])

click2 = st.selectbox("Что такое A/B-тестирование", ["", "Техника проверки гипотез. Позволяет оценить, как изменение сервиса или продукта повлияет на пользователей", "Результат деления выборки на две части", "Термин, используемый в статистике, означающий деление выборки на две части, для дальнейшего их сравнения"])

click3 = st.selectbox("Что такое A/A-тестирование", ["","Один из вариантов A/B-тестирования", "Способ проверить корректность работы механизма A/B-тестирования путем сравнения выборки самой с собой", "Тестирование, когда выборка B слишком мала"])

click4 = st.selectbox("Что предполагает нулевая гипотеза", ["", "Результаты B и A отличаются", "Результаты, А и В на самом деле не отличаются и что наблюдаемые различия случайны", "Результаты B и A отличаются, но это отличие не является статистически значимым"])

click5 = st.selectbox("Чем односторонний тест отличается от двустороннего", ["", "В одностороннем тесте не учитываются отрицательные значения в выборке", "У двустороннего теста нельзя посчитать уровень статистической значимости (α)", "Числом направлений, по которым мы хотим обноружить изменения"])

click6 = st.selectbox("За что отвечает уровень статистической значимости (α)", ["", "За уровень риска, который вы принимаете при ошибках первого рода", "За уровень риска, который вы принимаете при ошибках второго рода", "За уровень риска, который вы принимаете, вне зависимости от ошибок первого и второго рода"])

click7 = st.selectbox("Определите минимальный временные рамки для проведения A/B-теста на некотором сайте, если его ежедневный трафик составляет 10000 человек, а суммарный объем выборки, в данном эксперименте, 200000 человек", ["", "20 дней", "30 дней", "10 дней"])

click8 = st.selectbox("Чему равно значение Z-теста при среднем значение в выборке 1500, среднем значении в генеральной совокупности 2180, дисперсией 150 и объёме выборки 2500", ["", "-230", "148", "-226.6"])

count = 0
if click0 == "Метрика в интернет-маркетинге. Определяется как отношение числа кликов на баннер или рекламное объявление к числу показов":
    count += 1
    
if click1 == "Процент пользователей, которые совершили целевое действие по отношению к общему числу посетителей":
    count += 1
    
if click2 == "Техника проверки гипотез. Позволяет оценить, как изменение сервиса или продукта повлияет на пользователей":
    count += 1
    
if click3 == "Способ проверить корректность работы механизма A/B-тестирования путем сравнения выборки самой с собой":
    count += 1
    
if click4 == "Результаты, А и В на самом деле не отличаются и что наблюдаемые различия случайны":
    count += 1
    
if click5 == "Числом направлений, по которым мы хотим обноружить изменения":
    count += 1
    
if click6 == "За уровень риска, который вы принимаете при ошибках первого рода":
    count += 1
